...
if os.name == "nt":
    ...
    JOURNAL_FILE = r"C:\Program Files (x86)\ossec-agent\active-response\ai-disable-user.jsonl"
else:
    LOG_FILE = "/var/ossec/logs/active-responses.log"
    JOURNAL_FILE = "/var/ossec/logs/ai-disable-user.jsonl"
...
def validate_username(username):
    ...
    if os.name != "nt":
        if not re.fullmatch(r"[a-zA-Z_][a-zA-Z0-9._-]{0,31}", username):
            raise ValueError("Invalid Linux username format")
        if username == "root":
            raise ValueError("Refusing to disable root")
    else:
        if username.lower() == "administrator":
            raise ValueError("Refusing to disable Administrator")

    return username


def handle_linux_disable(username):
    exists = run_command(["id", username])
    if exists.returncode != 0:
        raise RuntimeError(f"User does not exist: {username}")

    lock_result = run_command(["usermod", "-L", username])
    if lock_result.returncode != 0:
        raise RuntimeError(lock_result.stderr.strip() or lock_result.stdout.strip())

    run_command(["pkill", "-KILL", "-u", username])


def handle_linux_enable(username):
    exists = run_command(["id", username])
    if exists.returncode != 0:
        raise RuntimeError(f"User does not exist: {username}")

    unlock_result = run_command(["usermod", "-U", username])
    if unlock_result.returncode != 0:
        raise RuntimeError(unlock_result.stderr.strip() or unlock_result.stdout.strip())


def handle_windows_disable(username):
    result = run_command(["net", "user", username, "/active:no"])
    if result.returncode != 0:
        raise RuntimeError(result.stderr.strip() or result.stdout.strip())


def handle_windows_enable(username):
...
def main(argv):
    ...
    try:
        if os.name == "nt":
            if effective_command == ADD_COMMAND:
                handle_windows_disable(username)
                event_name = "disable_user_success"
            else:
                handle_windows_enable(username)
                event_name = "enable_user_success"
        else:
            if effective_command == ADD_COMMAND:
                handle_linux_disable(username)
                event_name = "disable_user_success"
            else:
                handle_linux_enable(username)
                event_name = "enable_user_success"

        write_debug_file(
            argv[0],
            f"Completed successfully | user={username} | incoming_command={msg.command} | effective_command={effective_command}"
        )
